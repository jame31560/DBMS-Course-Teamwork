...
<body>
  <div class="container">
    ...
    if ($isUpdate) {
      $deliveryStaffID = $_GET["update"]; // update this on where
      $name = $_GET["name"];
      $tel = $_GET["tel"];
      // update code in here;
      ...
      mysqli_close($link);
      // header("Location: deliverystaff.php?msg=-1".(($isQuery)?"&query=".$_GET["query"]:""));
      header("Location: deliverystaff.php?msg=0".(($isQuery)?"&query=".$_GET["query"]:""));
    }
  ...
  ?>
    ...
    <form class="form-inline" action="deliverystaff.php" method="GET">
      ...
      <?php 
      if ($isQuery) {
        echo '<button type="reset" class="btn btn-secondary mb-2 mr-2" onClick="document.location=\'deliverystaff.php\';">取消查詢</button>';
      } 
    ?>
    </form>
        ...
        <tbody>
          <?php
            ...
            if (mysqli_num_rows($result) == 0) {
              echo '<tr><td class="align-middle text-center" colspan="5">查無結果</td></tr>';  //欄位數
            } else {
              while($row = mysqli_fetch_assoc($result)) {
                if ($isEdit && $editID == $row["deliveryStaffID"]) {
                  //編輯中
                  echo '<tr>
                    <th class="align-middle text-center" scope="row">'.$row["deliveryStaffID"].'</th>
                    ...
                    <td class="align-middle text-center" colspan="2">
                      <button class="btn btn-success btn-sm btn-block" type="submit" name="update" value="'.$row["deliveryStaffID"].'">
                        送出
                      </button>
                    </td>
                  </tr>';
                  ...
                  echo '<tr>
                    ...
                    <td class="align-middle text-center">
                      <button class="btn btn-warning btn-sm btn-block" type="submit" name="edit" value="'.$row["deliveryStaffID"].'">
                        編輯
                      </button>
                    </td>
                    <td class="align-middle text-center">
                      <button class="btn btn-danger btn-sm btn-block" type="submit" name="delete" value="'.$row["deliveryStaffID"].'">
                        刪除
                      </button>
                    </td>
                  </tr>';
